...
GUARDRAIL A (2026-09-08 — anti-write-verb enforcement): the harness
auto-injects `-exclude-tags dast,fuzz,intrusive,brute-force,form-fuzz,
injection,unauth-write,dos` into the nuclei call. `takeover` templates
are read-only by nature (DNS lookup + HTTP GET for the fingerprint
strings), so the injection does not affect functionality; it's a safety
net against future template additions that regress toward mutating
checks. Do NOT try to bypass with `# harness-allow-writes` — that
sentinel is reserved for the operator's lab-only runs, never for the
harness pipeline.
