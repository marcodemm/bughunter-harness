...
- **Nuclei tag rejector (shell layer)** — the web_vuln agent's prompt already lists `NON_SCANNABLE_TECHS` that have no server-side CVE templates (CDNs, trackers, frontend libs), but the LLM occasionally still emits `nuclei -tags cve,cloudflare` and other zero-hit combinations. `tools.py::maybe_reject_nuclei_scan` intercepts those before spending nuclei runtime and returns a clear ERROR the LLM sees on the next turn ("no server-side CVE templates in nuclei-templates"). Plain `-tags <tech>` without `cve` is allowed (tech-detect templates are useful).
