...
- **wpscan diagnostic visibility** — when `wpscan` or `nuclei` exit non-zero, the per-agent Tool Activity log carries a `stderr: …` excerpt (up to ~400 chars) so the operator can diagnose without re-running. Additionally, when `wpscan` returns exit=4 ("not a WordPress site") on a target where nuclei-wordpress-detect already confirmed WordPress, `state.wpscan_waf_suspect` is set and a meta-check warning appears in REPORT.md: "wpscan reported the target as 'not a WordPress site' while nuclei-wordpress-detect confirmed it IS WordPress" — followed by remediation steps (attribution header verification, `--verbose --debug` inspection, `curl-impersonate` fallback). If `curl-impersonate` / `curl_chrome116` is installed locally (checked at startup by the pre-check), the meta-check tells you `installed on this host — try curl_chrome116 -sI …`; if not, it gives the `brew install curl-impersonate` pointer.
- **wpscan 0-findings auto-diagnostic (4 causes)** — when `wpscan` runs but the JSON parser adds 0 findings, `wordpress.after_run` classifies the failure and publishes ONE of these flags so the report meta-check names the exact cause: (1) `wpscan_api_exhausted` (daily WPScan API quota hit, existing "WPScan API daily quota reached" warning fires); (2) `wpscan_waf_suspect` (target returns "not a WordPress site" while fingerprint confirmed WordPress — WAF/JA3 blocking, see previous bullet); (3) `wpscan_output_empty` (every wpscan call errored before writing the JSON file — likely rate-limit / network fail); (4) `wpscan_zero_findings` (files exist and non-empty but parser found no plugin/theme/core CVEs — could be genuine or WPScan API token missing). The agent no longer stays silent when the plugin-CVE lead evaporates.
- **WordPress REST user-enumeration auto-parse** — the `auth` agent's `after_run` deterministically scans every `curl` / `http_get` result whose command/url hits `/wp-json/wp/v2/users` (or `/?author=<n>` redirects). If the response body carries the classic JSON array `[{"id":<n>,"slug":"…"}]`, an INFO finding lands in REPORT.md listing the user count + first N slugs — with recommendation text explaining that each slug is a valid login username (feed for follow-on brute-force / password-spray). Idempotent — skips if the LLM's own `finish()` summary already emitted the same finding. Fires from both `run_shell` and `http_get` paths, so the finding survives whichever tool the LLM picked.
- **Implausible tech version ceilings — periodically refreshed** — the `_IMPLAUSIBLE_TECH_RANGES` table in `report.py` (last reviewed 2026-09-04 against official vendor release pages) sets a plausible max version per tech so a bad-detect like `WordPress:999.9` fires a meta-check warning. Ceilings sit 2-4 major releases above the current version so normal upstream progress doesn't false-positive. The check walks BOTH `state.detected_techs` and `live_hosts[i].tech` (the display-form list populated by recon's httpx `-tech-detect` fallback) — an implausible version fires the warning regardless of which source produced it. Re-review the table every 6 months and bump the ceilings before they turn into false positives.
