...
def _headers_reminder(custom_headers: dict[str, str]) -> str:
    ...
    lines.append(f'  curl -sI {dash_h} https://example.com/')
    lines.append(f'  nuclei -u https://example.com {dash_h} -tags cve -silent')
    lines.append(f'  wpscan --url https://example.com --headers "{wpscan_headers}" '
        ...
        while self.turns < max_iterations_run:
            ...
            for tc in msg.tool_calls:
                ...
                if name == "run_shell":
                    ...
                    # GUARDRAIL A (2026-09-08) — nuclei anti-write-verb
                    # enforcement. Runs BEFORE dispatch so:
                    #   (a) blocked template IDs get refused with a
                    #       helpful error before nuclei starts;
                    #   (b) auto-injected `-exclude-tags` appears in the
                    #       activity log (what bash actually ran).
                    # Incident that motivated this: 2026-09-08, nuclei
                    # `-tags wordpress` template for Breeze plugin issued
                    # POST /wp-comments-post.php → WordPress persisted an
                    # anonymous comment into the moderation queue on the
                    # researcher's own site. See tools.py header block.
                    try:
                        new_cmd, note, err = \
                            self.tool_registry \
                                .maybe_inject_nuclei_safe_flags(
                                    args.get("command", ""))
                        if err:
                            # Blocked template — record + surface to LLM
                            # without executing the command. Increments
                            # counters used by report.py meta-check (o).
                            state.log(self.NAME, "harness-inject", err)
                            red = redact(err)
                            transcript.append({
                                "tool": name, "args": args, "result": red,
                            })
