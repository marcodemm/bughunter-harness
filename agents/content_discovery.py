...
`example.com` / `example.org` / other RFC 2606 placeholders as if they
were the real target. If you're unsure what the target is, copy the
"Primary host" verbatim from the user message.
...
SHELL SYNTAX — CRITICAL (repeated denylist violations kill turn budget):
  ...
  the same pattern — SWITCH to the sequential form on the next turn:
    ...
    WRONG:  gau example.com > /tmp/x.txt && wc -l /tmp/x.txt
    RIGHT:  gau example.com > /tmp/x.txt ; wc -l /tmp/x.txt
...
"""
    ...
    def after_run(self, state, transcript):
        ...
        # must not eat `/fuzzy-search`, `/example.com/` must be a
        ...
        # C3 fix (2026-09-08) — filter LLM URL-invention.
        # Symptom seen in a real incident REPORT: the content_discovery
        # LLM emitted URLs shaped like
        #   https://<target>/%5C/xssnow.in%5C/payloads.html
        # where `xssnow.in` (an example of a public XSS payloads repo
        # the model saw during training) shows up inside the target's
        # path. `%5C/` is a backslash escape artifact from the model
        # cargoculting a wordlist entry. Path passes _in_scope() because
        # the host IS in scope, wastes probes and pollutes
        # `probed_negative` in the report.
        # Rule: drop URLs whose path contains a `%5C/`, `\/`, `\\/`, or
        # embeds a 3rd-party domain name (`.com/`, `.net/`, etc.) after
        # the first path separator — the LLM is quoting a payload URL,
        # not deriving from real crawl data.
        _LLM_INVENTION_HOST_MARKERS = re.compile(
            r"%5[Cc][\/]|\\{1,2}/|"
            r"/(?:[a-z0-9-]+\.)+(?:com|net|org|io|dev|sh|xyz|"
            r"me|co|it|de|uk|fr|es|nl|ru|pro|top|"
            r"in|to|ly|st|so|us|app|site|space)(?:/|$)",
            re.IGNORECASE,
        )

        def _looks_llm_invention(u: str) -> bool:
            try:
                p = _up(u)
            except Exception:
                return True
            ...
            if not path or path == "/":
                return False
            # 3rd-party domain embedded in the path → LLM cargocult.
            # Excludes the target's own hostname (that's just a self-
            # reference and fine).
            host = (p.hostname or "").lower()
            m = _LLM_INVENTION_HOST_MARKERS.search(path)
            if not m:
                return False
            # If the matched substring IS the target's own host, keep it.
            if host and host in m.group(0).lower():
                return False
            return True
        ...
        def _harvest_urls_from_text(text: str) -> None:
            for m in re.finditer(r"https?://[^\s\"'<>]+", text):
                ...
                if _looks_truncated(u):
                    ...
                    continue
                if _looks_llm_invention(u):
                    nonlocal_stats["llm_invention"] += 1
                    continue
                if _in_scope(u):
                    found.add(u)
                else:
        ...
        dropped_llm_invention = nonlocal_stats["llm_invention"]
        ...
        if dropped_llm_invention:
            state.log(self.NAME, "info",
                      f"C3 filter: dropped {dropped_llm_invention} URL(s) "
                      f"that embed a 3rd-party domain (e.g. xssnow.in, "
