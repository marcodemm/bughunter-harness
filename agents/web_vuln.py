...
CRITICAL for EVERY nuclei call: append
      -jsonl -o /tmp/harness-nuclei-webvuln-<N>.jsonl
where <N> is a monotonically increasing counter YOU pick per invocation
(1, 2, 3, 4, …). The suite below runs nuclei up to 4 times back-to-back
against the SAME target; if all 4 calls share the same `-o` file the
last call OVERWRITES the previous three → ingest sees 0 findings even
when templates fired. B1 fix (2026-09-08): each call MUST write to its
own file. The harness ingester glob-matches `harness-nuclei-webvuln*.jsonl`
so counter-suffixed files all get parsed. Do NOT use `>>` — nuclei's
JSONL output isn't safe to append (line boundaries can splice on
concurrent runs). Use per-call counters instead.

GUARDRAIL A (2026-09-08 — anti-write-verb enforcement, real incident):
the harness auto-injects `-exclude-tags dast,fuzz,intrusive,brute-force,
form-fuzz,injection,unauth-write,dos` into every nuclei stage. You can
add your own extra excludes; the harness merges. NEVER pass `-id` for
templates whose detection logic issues POST/PUT/DELETE against the
target — the harness maintains a short block list (see tools.py
NUCLEI_BLOCKED_TEMPLATE_IDS) and refuses those with a helpful error.
Reason: on 2026-09-08 a `wordpress-breeze` template's POST
/wp-comments-post.php was persisted by WordPress as an anonymous
comment into the moderation queue on the researcher's own site.
Passive tech-detect + wpscan --enumerate replace those templates
safely.
...
the actual detected_techs (max 3 combined).
  ...
  1. Generic exposures (always, first):
       nuclei -u <host> -tags exposures,exposed-panels,misconfig \
         ...
         -silent -jsonl -o /tmp/harness-nuclei-webvuln-1.jsonl
  ...
  2. Security headers:
       nuclei -u <host> -t http/misconfiguration/http-missing-security-headers.yaml \
         -silent -jsonl -o /tmp/harness-nuclei-webvuln-2.jsonl
  ...
  3. Default credentials scan (LAB ONLY — DVWA/Juice Shop/Mutillidae/BWA/
     self-hosted lab). Skip against real bug-bounty targets:
       nuclei -u <host> -tags default-login -severity medium,high,critical \
         ...
         -jsonl -o /tmp/harness-nuclei-webvuln-3.jsonl
  ...
  4. Per-tech CVE scan — one call per tech in detected_techs (max 3 techs).
     ...
     Use `-o /tmp/harness-nuclei-webvuln-4a.jsonl`, `-webvuln-4b.jsonl`,
     `-webvuln-4c.jsonl` for the 3 per-tech calls (letter suffix keeps
     each tech's output isolated):
       nuclei -u <host> -tags cve,<tech-lowercase> -severity medium,high,critical \
         -c 25 -rl 20 -timeout 5 -silent \
         -jsonl -o /tmp/harness-nuclei-webvuln-4<letter>.jsonl
...
"""
    ...
    def after_run(self, state, transcript):
        ...
        # is instructed to emit `-jsonl -o /tmp/harness-nuclei-webvuln-<N>.jsonl`
        # (B1 fix 2026-09-08 — per-call counter, was single overwriting
        # file before; incident details in SYSTEM_PROMPT) so nuclei's own
        # machine-readable output is authoritative. This eliminates the
        # dependency on stdout survival through the shell wrapper (bug B3
        # was hiding stdout output as "(no exit)" — B5 bypasses that
        # channel entirely for nuclei findings). Glob matches every
        # counter-suffixed file.
        try:
...
def _parse_nuclei_jsonl_to_findings(state, agent_name: str,
    ...
    # D3 fix (2026-09-08) — accumulate ingest stats in shared state so
    # `report.py` can render the Nuclei ingest self-check line + fire
    # meta-check `(s)` when the ratio is anomalous.
    try:
        counts = _count_nuclei_jsonl_lines(glob_pattern)
        prev = state.get("nuclei_ingest_stats") or {}
        stats = {
            "total_lines": int(prev.get("total_lines", 0))
                             + int(counts.get("total_lines", 0)),
            "total_matches": int(prev.get("total_matches", 0))
                               + int(counts.get("total_matches", 0)),
            "findings_added": int(prev.get("findings_added", 0)) + added,
            "total_files": int(prev.get("total_files", 0))
                            + len(counts.get("files", []) or []),
        }
        state.set("nuclei_ingest_stats", stats)
    except Exception:
