...
# ── Signal (B3 fix 2026-09-08): custom REST namespaces detected ─────
# WHY: WordPress + REST está en todos los CMS modernos. Cada plugin/tema
# con backend registra su propio namespace (`myplugin/v1`, `acme/v1`,
# `acme-license/v1`). Ese código es propio del target → sin cobertura CVE
# → alta densidad de bugs de authz/BOLA/BFLA/mass-assignment. La
# incidencia canónica que motivó este signal: el subsite con el webhook
# de Apple Server Notifications (`apple/notifications/v2`) fue rankeado
# MEDIUM (28) porque el scorer solo miraba tech/name; el subsite
# "presentacional" ganó (63 CRITICAL) sin surface real de código propio.
# Con este signal el ranking se invierte correctamente. Populate con
# `_prefetch_wp_json_for_wp_hosts` — corre justo antes del scoring en
# `SubPrioritizerAgent.run()`.
def _score_custom_rest_namespace(
    hostname: str, rest_cache: dict | None
) -> tuple[int, list[str]]:
    """Return (score, reasons) for custom REST namespaces on this host.

    `rest_cache` is `state.wp_rest_scoring_cache` — {host: {custom:[...],
    verbs:{POST|PUT|DELETE:count}, sensitive_routes:[...]}} populated
    by _prefetch_wp_json_for_wp_hosts().
    """
    if not rest_cache:
        return 0, []
    info = rest_cache.get(hostname)
    if not info:
        return 0, []
    custom = info.get("custom") or []
    if not custom:
        return 0, []
    score = 30  # base bonus for having ≥1 custom namespace
    reasons = [f"REST namespace CUSTOM x{len(custom)} "
               f"({', '.join(custom[:3])}{'…' if len(custom) > 3 else ''}) "
               f"= target's own code, high bug-hunting ROI (+30)"]
    verbs = info.get("verbs") or {}
    write_verb_count = sum(
        v for k, v in verbs.items()
        if str(k).upper() in ("POST", "PUT", "DELETE", "PATCH")
    )
    if write_verb_count > 0:
        score += 10
        reasons.append(
            f"custom namespaces expose {write_verb_count} write-verb "
            f"route(s) — state-mutation surface (+10)")
    sensitive = info.get("sensitive_routes") or []
    if sensitive:
        score += 10
        reasons.append(
            f"sensitive route pattern(s) in custom namespace "
            f"({', '.join(sensitive[:3])}{'…' if len(sensitive) > 3 else ''}) "
            f"— classic bug surface (webhook / download / upload / "
            f"callback) (+10)")
    return score, reasons


# ── Combine ─────────────────────────────────────────────────────────
def score_host(host_record: dict,
               in_scope_hosts: set[str] | None = None,
               wp_rest_cache: dict | None = None) -> dict:
    ...
    # B3 fix (2026-09-08) — REST custom namespace signal.
    rn_s, rn_r = _score_custom_rest_namespace(hostname, wp_rest_cache)
...
# ─────────────────────────────────────────────────────────────────────
# B3 (2026-09-08) — mini-prefetch of /wp-json/ for scoring.
#
# Timing: sub_prioritizer runs BEFORE fingerprint + wordpress. So
# `state.wp_rest_namespaces` (populated by wordpress agent's after_run)
# is NOT available at scoring time. Instead we do a fast, throttled
# fetch of `/wp-json/` for every live host that (a) is in scope,
# (b) is NOT in the researcher-infra allowlist, and (c) LOOKS WordPress
# by tech or title heuristics. Results go to `state.wp_rest_scoring_cache`
# and are picked up by `_score_custom_rest_namespace`.
# ─────────────────────────────────────────────────────────────────────

# ── B4 helper (2026-09-08) — researcher-infra allowlist matcher ─────

def _resolve_researcher_allowlist(
    cfg: dict, live: list[dict]
) -> tuple[set[str], list[str]]:
    """Return (excluded_hostnames, active_globs). `excluded_hostnames` is
    the set of hostnames from `live` that match any glob in
    `cfg.researcher_infra_allowlist`. `active_globs` is the raw list of
    globs from cfg for logging.

    Glob syntax:
      - `oob.example.net`          exact hostname match
      - `*.example.net`            any hostname ending with `.example.net`
      - `*.oast.pro`               etc.
      - `example.net`              apex + any subdomain (implicit *.)

    Empty / missing config → returns (empty set, empty list). The default
    config.example.yaml documents the recommended list; users override
    per-project via config.yaml.
    """
    raw = cfg.get("researcher_infra_allowlist") or []
    if isinstance(raw, str):
        raw = [raw]
    if not isinstance(raw, list):
        return set(), []
    globs = [str(g).strip().lower() for g in raw if str(g).strip()]
    if not globs:
        return set(), []

    def _matches(host: str, glob: str) -> bool:
        if not host or not glob:
            return False
        if host == glob:
            return True
        if glob.startswith("*."):
            suffix = glob[1:]  # keep the leading dot
            return host.endswith(suffix)
        # Bare apex like "example.net" → match apex + any subdomain
        if host == glob or host.endswith("." + glob):
            return True
        return False

    excluded: set[str] = set()
    for h in live:
        hostname = str(h.get("host", "")).lower().split(":", 1)[0]
        if not hostname:
            continue
        for g in globs:
            if _matches(hostname, g):
                excluded.add(hostname)
                break
    return excluded, globs


_WP_SIGNALS_TECH = ("wordpress", "wp-", "wp_")
_WP_SIGNALS_TITLE = ("wordpress",)
_REST_SENSITIVE_PATTERNS = (
    "download", "upload", "webhook", "callback", "notification",
    "license", "activate", "deactivate", "purchase", "receipt",
    "invoice", "checkout", "payment", "refund", "credentials",
    "token", "impersonate", "reset", "signup", "invite",
)


def _looks_wordpress(host_record: dict) -> bool:
    techs = [str(t).lower() for t in
             (host_record.get("tech") or host_record.get("technologies") or [])]
    if any(any(s in t for s in _WP_SIGNALS_TECH) for t in techs):
        return True
    title = str(host_record.get("title") or "").lower()
    if any(s in title for s in _WP_SIGNALS_TITLE):
        return True
    return False


def _prefetch_wp_json_for_wp_hosts(
    live: list[dict],
    ...
    """For every live host that looks WordPress AND is not excluded,
    fetch `/wp-json/` and extract custom namespaces + write-verbs +
    sensitive route patterns. Returns {host: {custom, verbs,
    sensitive_routes}}. Best-effort; silently drops errors."""
    ...
    for h in live:
        if seen >= max_hosts:
            break
        hostname = str(h.get("host", "")).lower().split(":", 1)[0]
        if not hostname or hostname in excluded_hosts:
            continue
        if not _looks_wordpress(h):
            continue
        scheme = h.get("scheme") or "https"
        url = f"{scheme}://{hostname}/wp-json/"
        try:
        ...
        if not isinstance(namespaces, list):
            continue
        # Import the classifier from wordpress agent (loaded at pipeline time)
        try:
            from agents.wordpress import _is_core_wp_namespace as _is_core
        except Exception:
            def _is_core(ns: str) -> bool:  # local fallback
                return (not ns) or ns.startswith("wp-") or ns.startswith("wp/")
        custom = sorted({ns for ns in namespaces
                         if not _is_core(str(ns))})
        # Verb + sensitive-route inspection
        verbs: dict[str, int] = {}
        sensitive: list[str] = []
        ...
        if isinstance(routes, dict):
            for path, info in routes.items():
                ...
                if any(pat in path_low for pat in _REST_SENSITIVE_PATTERNS):
                    sensitive.append(str(path))
        out[hostname] = {
            "custom": custom,
            "verbs": verbs,
            "sensitive_routes": sorted(set(sensitive)),
        }
    return out
...
class SubPrioritizerAgent(BaseAgent):
    ...
    def run(self, state) -> str:
        ...
        try:
            ...
            # B4 fix (2026-09-08) — researcher-infra allowlist. Filter
            # hosts that match user's own OOB / callback / lab domains
            # BEFORE scoring so they never appear in the ranking table
            # and are also skipped by the /wp-json/ prefetch below.
            # See config.example.yaml → researcher_infra_allowlist.
            excluded_hosts, allowlist_globs = _resolve_researcher_allowlist(
                self.cfg, live)
            if excluded_hosts:
                live_kept = [h for h in live
                             if str(h.get("host", "")).lower().split(":", 1)[0]
                             not in excluded_hosts]
                excluded_records = [
                    h for h in live
                    if str(h.get("host", "")).lower().split(":", 1)[0]
                    in excluded_hosts
                ]
                state.set("researcher_infra_excluded_hosts",
                          [str(h.get("host", "")) for h in excluded_records])
                state.log(self.NAME, "info",
                          f"researcher-infra filter: excluded "
                          f"{len(excluded_hosts)} host(s) "
                          f"({', '.join(sorted(excluded_hosts))}) matching "
                          f"researcher_infra_allowlist "
                          f"({', '.join(allowlist_globs[:5])}"
                          f"{'…' if len(allowlist_globs) > 5 else ''}) — "
                          "see REPORT section 'Researcher infra excluded "
                          "from scope'.")
                live = live_kept
                # Persist filtered live_hosts so downstream agents don't
                # scan the researcher's own infra either.
                state.set("live_hosts", live)

            # B3 fix (2026-09-08) — mini-prefetch of /wp-json/ for the
            # WordPress hosts among `live` so score_host() can boost hosts
            # with custom REST namespaces (+30 base). Only touches hosts
            # that look WordPress (by tech / title) and are NOT excluded
            # by B4. Fast, throttled, best-effort — silently skips on
            # network errors.
            try:
                rest_cache = _prefetch_wp_json_for_wp_hosts(
                    live,
                    custom_headers=(self.cfg.get("custom_headers") or {}),
                    excluded_hosts=excluded_hosts,
                    timeout=float(self.cfg.get("http_timeout_sec", 4)),
                    max_hosts=int(self.cfg.get("wp_json_prefetch_max_hosts",
                                                12)),
                )
                if rest_cache:
                    state.set("wp_rest_scoring_cache", rest_cache)
                    state.log(self.NAME, "info",
                              f"/wp-json/ prefetch: probed "
                              f"{len(rest_cache)} WordPress host(s), "
                              f"found custom namespaces on "
                              f"{sum(1 for v in rest_cache.values() if v.get('custom'))} "
                              f"host(s)")
            except Exception as e:
                state.log(self.NAME, "warn",
                          f"/wp-json/ prefetch failed: "
                          f"{type(e).__name__}: {e}")
                rest_cache = {}

            scored = [score_host(h, in_scope_hosts=in_scope,
                                 wp_rest_cache=rest_cache) for h in live]
