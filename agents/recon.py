...
"""
    ...
    def after_run(self, state, transcript):
        ...
        for entry in transcript:
            ...
            # naabu → host:port
            if "naabu" in cmd:
                # C2 fix (2026-09-08) — capture naabu exit code so we can
                # emit an explicit caveat when the scan silently failed
                # (typical when the target/network drops SYN probes from
                # residential IPs). Nested inside the loop so it can key
                # per-command; aggregate at the end.
                _exit_match = re.search(r"exit=(\d+)", result)
                _exit_code = int(_exit_match.group(1)) if _exit_match else None
                _found_any = False
                for m in re.finditer(r"^([a-z0-9._-]+):(\d+)\s*$",
                    ...
                    _found_any = True
                if _exit_code and _exit_code != 0 and not _found_any:
                    # naabu bounced with 0 findings — publish caveat for
                    # report.py meta-check (n) so operator knows to try
                    # alternative (nmap -sT -Pn from a VPS).
                    _host_match = re.search(
                        r"-host\s+(\S+)|-hl?\s+(\S+)", cmd)
                    _host_hint = ""
                    if _host_match:
                        _host_hint = (_host_match.group(1)
                                       or _host_match.group(2) or "")
                    state.append("recon_naabu_failed_hosts", {
                        "host": _host_hint,
                        "exit": _exit_code,
                    })
