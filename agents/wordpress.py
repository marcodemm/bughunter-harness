...
Workflow (one tool_call per turn):
  ...
  0a. REST API surface enumeration (B2 fix 2026-09-08 — MANDATORY):
       curl -s <target>/wp-json/ | jq '.namespaces, (.routes | keys)'
     Parse the response to enumerate ALL registered REST namespaces.
     The harness expects this data in state for the sub_prioritizer
     scorer (a custom namespace is +30 pts, and rankings shifted
     accordingly). Filter core WP namespaces (wp/v2, oembed/1.0,
     wp-site-health/v1, wp-block-editor/v1, wp-abilities/v1, batch/v1,
     any prefix `wp-`); everything else is CUSTOM = target's own code =
     high-value bug-hunting surface. Save output to
     /tmp/harness-wp-json.json for the harness parser to pick up.
     If /wp-json/ returns 404 or HTML (not JSON), the REST API is
     disabled or firewalled; log it and continue with step 0b.
  ...
  0b. IMPORTANT — Cloudflare/WAF bypass:
     ...
     if steps 1-2 all return exit=4, jump directly to step 2b (curl-
     impersonate fallback) — do NOT keep retrying wpscan with variations.
  ...
  2b. curl-impersonate fallback (C4 fix 2026-09-08 — MANDATORY when
      steps 1-2 return exit=4). Run these probes with `curl_chrome116`
      (or `curl_firefox144` as second try if the site fingerprints
      Chrome specifically):
        curl_chrome116 -sI https://<target>/wp-login.php
        curl_chrome116 -sI https://<target>/wp-admin/
        curl_chrome116 -s  https://<target>/?rest_route=/wp/v2/users
      If `which curl_chrome116` returns exit=1, the binary is missing.
      RECOMMENDED install (2026-09-08 iter 18b — Homebrew fails on
      modern macOS due to CMake / Xcode version pinning): download the
      precompiled tarball from the actively-maintained fork:
        gh release download v2.2.2 \
          --repo lexiforest/curl-impersonate \
          --pattern 'curl-impersonate-v2.2.2.arm64-macos.tar.gz'
        tar -xzf curl-impersonate-*.tar.gz
        cp curl_* curl-impersonate /opt/homebrew/bin/
      Or on Linux: `apt install curl-impersonate` (Debian/Ubuntu).
      wpscan's default JA3 fingerprint is known-blocked by modern WAFs
      (Cloudflare, Sucuri, DataDome, Wordfence); the Chrome-impersonated
      curl is the only reliable way to reach the origin unauthenticated.
  ...
  3. nuclei -u <target> -tags wordpress -severity medium,high,critical \
       ...
       GUARDRAIL A (2026-09-08 write-verb incident): the harness
       auto-injects `-exclude-tags dast,fuzz,intrusive,brute-force,
       form-fuzz,injection,unauth-write,dos` so templates that verify
       via POST/PUT/DELETE (e.g. `wordpress-breeze` issuing POST
       /wp-comments-post.php) NEVER persist state against the target.
       You do NOT need to add `-exclude-tags` yourself; the injection
       is automatic and logged as `[harness-inject-guardrail-A]`.
  ...
  5. finish() with findings: plugins outdated, users leaked, CVEs matched,
     REST namespaces custom (list them separately from core WP).
...
"""
    ...
    def after_run(self, state, transcript):
        # B2 fix (2026-09-08) — parse /wp-json/ namespace enumeration.
        # Publishes `state.wp_rest_namespaces` = {core: [...],
        # custom: [...], routes: {path: {methods, endpoints, args}}}
        # so sub_prioritizer can boost hosts with custom namespaces
        # (+30 pts) and report.py can render the new REST section.
        try:
            _parse_wp_json_namespaces(state, self.NAME, transcript)
        except Exception as e:
            state.log(self.NAME, "warn",
                      f"/wp-json/ parse failed: {type(e).__name__}: {e}")

        # C4 fix (2026-09-08) — detect wpscan WAF-block (exit=4) and
        # verify the curl-impersonate fallback was attempted. If wpscan
        # was blocked but the LLM skipped step 2b, log a caveat for
        # the meta-check in report.py.
        try:
            _diagnose_wpscan_waf_needs_fallback(state, self.NAME, transcript)
        except Exception as e:
            state.log(self.NAME, "warn",
                      f"wpscan WAF-diagnosis failed: {type(e).__name__}: {e}")
...
# ─────────────────────────────────────────────────────────────────────────
# B2 (2026-09-08) — parse /wp-json/ to enumerate custom REST namespaces.
#
# WHY: WordPress moderno vive en REST. Cada plugin/tema con backend registra
# su propio namespace (`myplugin/v1`, `acme/v1`, `acme-license/v1`) con rutas
# `POST/PUT/DELETE`. Es la superficie de MÁXIMO valor para bug hunting:
# código propio del target = sin cobertura CVE, alto ROI para bugs de authz,
# BOLA/BFLA, mass-assignment, business logic. La incidencia canónica que
# motivó este pipeline: sub_prioritizer rankeaba el sitio "presentacional"
# (63, CRITICAL) sobre el subsite con licensing custom (28, MEDIUM) cuando
# el segundo es el que tiene `/wp-json/acme-license/v1/{license/*,apple/
# notifications/v2}` — el target de VALOR real.
#
# QUÉ HACE: lee /tmp/harness-wp-json.json (donde el LLM guardó el output
# de `curl -s <target>/wp-json/`), separa core WP de custom, publica en
# state.wp_rest_namespaces para que:
#   - sub_prioritizer.py sume +30 pts por namespace custom (B3)
#   - report.py renderice una sección nueva (D1)
# ─────────────────────────────────────────────────────────────────────────

_WP_REST_CORE_NAMESPACES = frozenset({
    "wp/v2", "oembed/1.0", "wp-site-health/v1", "wp-block-editor/v1",
    "wp-abilities/v1", "batch/v1",
})


def _is_core_wp_namespace(ns: str) -> bool:
    """True if `ns` is a WordPress-core REST namespace (not target's own
    code). Matches the fixed set + any `wp-*` prefix (defensive)."""
    if not ns:
        return True
    if ns in _WP_REST_CORE_NAMESPACES:
        return True
    if ns.startswith("wp-"):
        return True
    if ns.startswith("wp/"):
...
def _parse_wp_json_namespaces(state, agent_name: str, transcript) -> None:
    """Parse /wp-json/ REST output from either
       (a) /tmp/harness-wp-json.json (if the LLM saved it there), or
       (b) directly from a transcript entry whose command was
           `curl -s <target>/wp-json/`.

    Publishes to state:
      wp_rest_namespaces = {
          "target": "<host>",
          "core": [...],          # e.g. ['wp/v2', 'oembed/1.0']
          "custom": [...],        # e.g. ['acme/v1', 'acme-license/v1']
          "routes": {             # trimmed for report readability
              "<path>": {"methods": [...], "namespace": "...", "n_endpoints": N},
              ...
          },
      }
    Idempotent — silently no-ops if no data / already parsed.
    """
    ...
    # (a) preferred — file the LLM saved
    fpath = "/tmp/harness-wp-json.json"
    try:
    ...
    # (b) fallback — scan transcript for the curl output
    if not raw:
        for entry in transcript:
            if entry.get("tool") != "run_shell":
                continue
            cmd = str(entry.get("args", {}).get("command", ""))
            if "/wp-json/" not in cmd or "curl" not in cmd:
                continue
            result = str(entry.get("result", ""))
            # nuclei/wpscan use /wp-json/wp/v2/users; that returns a JSON
            # ARRAY of users, not the namespace root. Skip those.
            if '"namespaces"' not in result and '"routes"' not in result:
                continue
            raw = result
            source = "transcript"
            break
    ...
    # Trim to JSON object — LLM output may have curl HTTP status lines
    # before/after the body.
    try:
        first_brace = raw.index("{")
        last_brace = raw.rindex("}")
        obj_text = raw[first_brace:last_brace + 1]
        data = _json.loads(obj_text)
    except (ValueError, _json.JSONDecodeError):
        state.log(agent_name, "warn",
                  f"/wp-json/ payload from {source or 'unknown'} did not "
                  "parse as JSON — target likely returned an HTML error "
                  "page or WAF challenge. Falling back to prefetch cache.")
        _fallback_from_prefetch_cache(state, agent_name)
        return
    ...
    core = sorted({ns for ns in namespaces if _is_core_wp_namespace(ns)})
    custom = sorted({ns for ns in namespaces
                     if not _is_core_wp_namespace(ns)})

    # Routes: trim to (path, methods, namespace, n_endpoints) so the
    # report stays compact. Keep everything; the report can decide what
    # to render.
    routes_out: dict = {}
    raw_routes = data.get("routes") or {}
    if isinstance(raw_routes, dict):
        for path, info in raw_routes.items():
            ...
            routes_out[str(path)] = {
                ...
                "n_endpoints": len(eps) if isinstance(eps, list) else 0,
                "is_core": _is_core_wp_namespace(ns),
            }

    # Target hint — best-effort from the transcript curl command
    tgt = ""
    for entry in transcript:
        if entry.get("tool") != "run_shell":
            continue
    ...
    state.set("wp_rest_namespaces", payload)
    state.log(agent_name, "info",
              f"/wp-json/ parsed: {len(core)} core namespace(s), "
              f"{len(custom)} custom namespace(s) "
              f"({', '.join(custom[:5])}{'…' if len(custom) > 5 else ''}), "
              f"{len(routes_out)} route(s)")

    # BUG-A fix (2026-09-08 iter 18b) — cuando el parser final devuelve
    # 0 namespaces pero el prefetch de sub_prioritizer ya detectó
    # custom ones antes (aunque en formato diferente porque el LLM usó
    # `jq '.namespaces, (.routes | keys)'` que emite 2 JSONs separados
    # por newline), rescatar del cache del prefetch. Sin este fallback,
    # la sección D1 del REPORT queda ausente aunque las namespaces SÍ
    # se hubieran detectado. Detectado en un post-run analysis donde
    # el prefetch reportaba "found custom namespaces on 2 host(s)"
    # pero el parser final del agent wordpress reportaba
    # "0 core, 0 custom, 10 route(s)" — contradicción interna.
    if not core and not custom and not routes_out:
        _fallback_from_prefetch_cache(state, agent_name)


def _fallback_from_prefetch_cache(state, agent_name: str) -> None:
    """BUG-A fix (2026-09-08 iter 18b) — cuando `_parse_wp_json_namespaces`
    no consigue extraer nada de la ruta preferred (fichero temp) ni del
    fallback (transcript), rescatar los datos que `sub_prioritizer.py::
    _prefetch_wp_json_for_wp_hosts` ya persistió en
    `state.wp_rest_scoring_cache`. Ese cache es determinista (no depende
    del LLM), y si el prefetch corrió antes de sub_prioritizer, va a
    tener los custom namespaces por host.

    Convierte {host: {custom, verbs, sensitive_routes}} → payload con
    forma compatible con la sección D1 del REPORT.
    """
    cache = state.get("wp_rest_scoring_cache") or {}
    if not cache:
        state.log(agent_name, "warn",
                  "BUG-A fallback: no prefetch cache available either — "
                  "sub_prioritizer's /wp-json/ prefetch also came up "
                  "empty. Either the site's REST API is fully hardened "
                  "off (mu-plugin / .htaccess) or WAF is blocking both "
                  "the LLM's request and the prefetch. Consider running "
                  "with curl-impersonate manually.")
        return

    # Escoger el host con más custom namespaces como "target" nominal.
    # (Si hay varios hosts en el cache, todos importan; renderizamos
    # la unión para la sección D1.)
    all_custom = set()
    all_core = set()
    routes_out: dict = {}
    target_hint = ""
    for host, info in cache.items():
        for ns in info.get("custom") or []:
            all_custom.add(ns)
        if info.get("custom") and not target_hint:
            target_hint = host
        # El prefetch guardó sensitive_routes (lista de paths); no tiene
        # methods per-path, pero sí verbs agregados. Renderizamos las
        # sensitive_routes con métodos "?" para transparencia.
        for path in info.get("sensitive_routes") or []:
            routes_out[str(path)] = {
                "namespace": next(
                    (ns for ns in (info.get("custom") or [])
                     if str(path).startswith("/" + ns.split("/")[0])),
                    "?"),
                "methods": ["?"],  # el prefetch no persiste methods per-path
                "n_endpoints": 0,
                "is_core": False,
            }
    ...
    payload = {
        ...
        "custom": sorted(all_custom),
        "routes": routes_out,
        "source": "sub_prioritizer prefetch cache (BUG-A fallback)",
    }
    state.set("wp_rest_namespaces", payload)
    state.log(agent_name, "info",
              f"BUG-A fallback: recovered {len(all_custom)} custom "
              f"namespace(s) from sub_prioritizer prefetch cache "
              f"across {len(cache)} host(s) — the wordpress agent's "
              f"final parser found nothing, but the deterministic "
              f"prefetch had already captured them. Rendering D1.")


# ─────────────────────────────────────────────────────────────────────────
# C4 (2026-09-08) — detect wpscan WAF-block and check fallback attempt.
# ─────────────────────────────────────────────────────────────────────────

def _diagnose_wpscan_waf_needs_fallback(
    state, agent_name: str, transcript
) -> None:
    """If wpscan returned exit=4 (or "does not seem to be running
    WordPress"), verify the LLM attempted step 2b (curl-impersonate)
    before finishing. If not, publish a state flag for the meta-check.
    """
    import re as _re

    wpscan_blocked = False
    curl_fallback_attempted = False
    curl_impersonate_missing = False
    ...
    for entry in transcript:
        ...
        if "wpscan " in cmd:
            if (_re.search(r"exit=4(?!\d)", result)
                    or "does not seem to be running wordpress" in low_result):
                wpscan_blocked = True
        # curl_chrome116 / curl_ff109 / curl-impersonate-* attempted?
        if _re.search(r"^\s*curl[_-](chrome|ff|impersonate)", cmd):
            curl_fallback_attempted = True
            if _re.search(r"exit=127", result):
                curl_impersonate_missing = True

    if wpscan_blocked and not curl_fallback_attempted:
        state.set("wpscan_waf_blocked_no_fallback", True)
        state.log(agent_name, "warn",
                  "wpscan returned exit=4 (WAF block or NON-wordpress "
                  "page) but the curl-impersonate fallback (step 2b) was "
                  "NOT attempted. The LLM should have run "
                  "`curl_chrome116 -sI <target>/wp-login.php` to bypass "
                  "the JA3 fingerprint check. Meta-check will surface "
                  "this in the report.")
    if curl_impersonate_missing:
        state.set("curl_impersonate_missing", True)
        state.log(agent_name, "warn",
                  "curl-impersonate binary not installed. Install with "
                  "`brew install curl-impersonate` on macOS or "
                  "`apt install curl-impersonate` on Debian/Ubuntu. "
                  "Fallback for WAF-blocked wpscan is disabled until "
                  "then.")
